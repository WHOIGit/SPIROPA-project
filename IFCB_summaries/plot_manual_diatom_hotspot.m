...
for count = 1:length(ii)
    for count2 = 1:length(classes)
        ...
        if ~isempty(bv)
           ...
           cast_count_gt10(count,count2) = sum(esd>10);
           cast_count_gt5(count,count2) = sum(esd>5);
           cast_bv_all(count,count2) = sum(bv);
           cast_bv_gt10(count,count2) = sum(bv(esd>10));
           cast_bv_gt5(count,count2) = sum(bv(esd>5));
        end
    end
end
...
%non-zero biovol for diatoms with esd>5
...
for cc = 1:length(castset)
  ...
  %  castii = find(strcmp(cast_table_binned.cast, caststr) & strcmp(cast_table_binned.cruise, cruisestr));
    ...
    subplot(1,2,1)
    barh(cast_table_binned.depth(castii(ids)), sum2plot(castii(ids),:), 'stacked', 'barwidth', 4/min(diff(cast_table_binned.depth(castii(ids)))))    
    set(gca, 'ydir', 'rev')
    title([cruisestr ' station ' num2str(castset(cc))])
    ylabel('Depth (m)'), 
    axis([0 1.8e6 0 60])
