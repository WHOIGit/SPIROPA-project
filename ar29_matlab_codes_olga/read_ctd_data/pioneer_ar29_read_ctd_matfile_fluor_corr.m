% pioneer_ar29_read_ctd_matfile_fluor_corr.m
% From: pioneer_ar29_read_ctd_matfile.m
...
elseif strcmp(data_location,'olgas_path')
    path_name='~/OOI_pioneer/p_11/leg_1/ctd/process_final_fluor_corr/ctd_matfile/';
end
path_name_txt=strrep(path_name,'_','\_');
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
%@@@@@@@@@@@@@@@@@@  FILE NAME  @@@@@@@@@@@@@@@@@@@@@@
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
fname='ar29_ctd_data_fluor_corrected';
...
%check_with_old='yes';
check_with_old='no';
if strcmp(check_with_old,'yes')
    d_new=d;
    load ~/OOI_pioneer/p_11/leg_1/ctd/process_final/ctd_matfile/ar29_ctd_data;
    d_old=data;
    clear data;
    % Remove Fluor - known as not equal:
    d_old(:,8)=[];
    d_new(:,8)=[];
    sum_d=sum(d_old(:) - d_new(:));
    if sum_d~=0
        disp('!!! Data matrices are not equal');
    else
        disp('Data matrices are equal');
    end
    clear d_old d_new;
    return;
end
...
if strcmp(show_stations_options,'All and One')
    ...
    for castNum=cast_list
        ...
        cast_total_points=length(parSt);
        %--------------------------
        % Remove "bad" data.
        %--------------------------
        ...
        if ~isempty(r_bad)
            ...
            %%%total_bad=[total_bad;castNum*ones(size(r_bad)) presSt(r_bad)];

            str_disp=strvcat(str_disp,['Cast No ' num2str(castNum) ', ' jN ': ' num2str(length(r_bad)) ' line(s) with "bad" data (flag=-9.990e-29)']);
            %str_disp=strvcat(str_disp,['Cast No ' num2str(castNum) ', ' jN ': ' num2str(length(r_bad)) ' line(s) with "bad" data (flag=-9.990e-29)'],...
            %    'Pressure  Quantity',num2str([presSt(r_bad,:) parSt(r_bad,:)]));
        ...
        if ~isempty(parSt)
            ...
            cast_all_bad=[cast_all_bad;castNum cast_total_points];
        end
...
if size(str_disp,1)>1
    disp(str_disp);
end
if ~isempty(cast_all_bad), cast_all_bad, end
